(defun print-coords (coords)
  (let ((str "")
        (max-x (seq-reduce (lambda (acc c) (max acc (first c))) coords 0))
        (max-y (seq-reduce (lambda (acc c) (max acc (second c))) coords 0)))
    (dotimes (y (1+ max-y))
      (dotimes (x (1+ max-x))
        (if (member (list x y) coords)
            (setq str (concat str "#"))
          (setq str (concat str " "))))
      (setq str (concat str "\n")))
    (message str)))
